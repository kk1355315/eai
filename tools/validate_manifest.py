...
def validate_manifest(manifest_path: Path, taxonomy_path: Path) -> ValidationResult:
    ...
    with manifest_path.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            resolved_image_path = resolve_image_path(manifest_path, image_path)

            if not image_path:
                errors.append(f"row {row_number}: image_path is empty")
            elif not resolved_image_path.exists():
                errors.append(f"row {row_number}: image_path '{image_path}' does not exist")
            ...
            dedupe_key = str(resolved_image_path.resolve())
