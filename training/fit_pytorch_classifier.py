...
def resolve_split_paths(prepared_dir: Path, use_mixed_train: bool = False) -> dict[str, Path]:
    train_name = "train_mixed.csv" if use_mixed_train else "train.csv"
    return {
        "train": prepared_dir / train_name,
        "val": prepared_dir / "val.csv",
        "test": prepared_dir / "test.csv",
        "label_map": prepared_dir / "label_map.json",
        "summary": prepared_dir / "summary.json",
    }
...
def fit_classifier(args: argparse.Namespace) -> FitResult:
    prepared_dir = prepare_inputs(args)
    split_paths = resolve_split_paths(
        prepared_dir=prepared_dir,
        use_mixed_train=args.stage == "stage_b" and args.mix_public_train,
    )
    ...
    label_map = read_label_map(split_paths["label_map"])
    class_names = [str(entry["class_name"]) for entry in label_map]
    label_to_index = {name: index for index, name in enumerate(class_names)}

    train_rows = read_manifest_rows(split_paths["train"])
    val_rows = read_manifest_rows(split_paths["val"])
    test_rows = read_manifest_rows(split_paths["test"])
    ...
    prepared_summary = json.loads(split_paths["summary"].read_text(encoding="utf-8"))
    metrics = {
        "stage": args.stage,
        "model_name": args.model_name,
        "weights": args.weights,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prepare normalized splits and fit a PyTorch image classifier."
    )
    subparsers = parser.add_subparsers(dest="stage", required=True)

    stage_a = subparsers.add_parser("stage_a", help="Fit a public-data Stage A classifier")
    ...
    stage_a.add_argument(
        "--fit-output-dir",
        type=Path,
        default=Path("training/runs/stage_a_fit"),
        help="Directory where fitted Stage A artifacts will be written",
    )

    stage_b = subparsers.add_parser("stage_b", help="Fit a fridge-domain Stage B classifier")
    ...
    stage_b.add_argument(
        "--fit-output-dir",
        type=Path,
        default=Path("training/runs/stage_b_fit"),
        help="Directory where fitted Stage B artifacts will be written",
    )
    stage_b.add_argument(
        "--mix-public-train",
        action="store_true",
        help="Append public train rows into Stage B train data",
    )

    for stage_parser in (stage_a, stage_b):
